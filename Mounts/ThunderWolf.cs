...
{
	public class ThunderWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<ThunderDust>();
			mountData.buff = mod.BuffType("ThunderWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<ThunderDust>());
		}

		internal class ThunderWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public ThunderWolfSpecificData()
			{
				count = 3;
