...
{
	public class AirWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<AirDust>();
			mountData.buff = mod.BuffType("AirWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<AirDust>());
		}

		internal class AirWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public AirWolfSpecificData()
			{
				count = 3;
