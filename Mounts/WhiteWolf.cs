...
{
	public class WhiteWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<AstralMoonDust>();
			mountData.buff = mod.BuffType("WhiteWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<AstralMoonDust>());
		}

		internal class WhiteWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public WhiteWolfSpecificData()
			{
				count = 3;
