...
{
	public class FireWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<FireDust>();
			mountData.buff = mod.BuffType("FireWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<FireDust>());
		}

		internal class FireWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public FireWolfSpecificData()
			{
				count = 3;
