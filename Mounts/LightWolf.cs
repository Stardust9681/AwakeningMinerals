...
{
	public class LightWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<LightDust>();
			mountData.buff = mod.BuffType("LightWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<LightDust>());
		}

		internal class LightWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public LightWolfSpecificData()
			{
				count = 3;
