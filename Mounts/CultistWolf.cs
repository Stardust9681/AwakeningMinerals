...
{
	public class CultistWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<IceDust>();
			mountData.buff = mod.BuffType("CultistWolfBuff");
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<IceDust>());
		}

		internal class CultistWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public CultistWolfSpecificData()
			{
				count = 3;
