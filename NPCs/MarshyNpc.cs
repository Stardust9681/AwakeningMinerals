...
namespace AwakeningMinerals.NPCs
{
	...
	[AutoloadHead]
	public class MarshyNpc : ModNPC
	{
		public override string Texture => "AwakeningMinerals/NPCs/MarshyNpc";
		...
		public override bool CanTownNPCSpawn(int numTownNPCs, int money) {
			for (int k = 0; k < 255; k++) {
				...
				foreach (Item item in player.inventory) {
					if (item.type == ModContent.ItemType<Items.Classes.EmptyDisk>()) {
						return true;
					}
				}
			}
		...
		public override string GetChat()
		{
			switch (Main.rand.Next(3)) {
				case 0:
					return "Ehy, you need something?.";
				case 1:
					return "Opa";
				default:
					return "You need pets?";
			}
		}
		...
		*/
		...
		{
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<Items.Pets.MarshyButterfly>());
			nextSlot++;
		...
		{
			projType = ModContent.ProjectileType<PinkSpike>();
			attackDelay = 1;
		}
