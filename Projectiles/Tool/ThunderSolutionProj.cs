...
using AwakeningMinerals.Walls.ElementalBiomes.ThunderBiome;
...
{
	public class ThunderSolutionProj : ModProjectile
	{
		...
		public override void AI() {
			...
			int dustType = ModContent.DustType<Dusts.ThunderSolutionDust>();
			...
			for (int k = i - size; k <= i + size; k++) {
				for (int l = j - size; l <= j + size; l++) {
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size)) {
						...
						if (wall != 0) {
							Main.tile[k, l].wall = (ushort)ModContent.WallType<ThunderStoneWall>();
						...
						if (TileID.Sets.Conversion.Stone[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<ThunderStoneTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						//If the tile is sand, convert to ExampleSand
						else if (TileID.Sets.Conversion.Grass[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<ThunderGrassTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
												//If the tile is sand, convert to ExampleSand
						else if (TileID.Sets.Conversion.Sand[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<ThunderSandTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
					}
